...
import { assert, ensure } from "../shared/assertion";
...
export default class NeuralNetwork extends NeuralNetworkBase {
  constructor({
    id,
    optimizer = "adam",
  ...
  async _getModel(inputCount, outputCount) {
    ...
    m.add(
      tf.layers.dense({
        ...
        activation:
          this.inputActivation === "leakyrelu"
            ? undefined
  ...
  async *train({
    ...
    if (randomize) {
      ...
      data = data.map((d) => ({
        ...d,
        x: new Array(inputCount).map(() => (Math.random() > 0.5 ? 1 : 0)),
      }));
    }
    ...
    const val_xs =
      validationData && tf.tensor2d(validationData.map((d) => d.x));
    const val_ys =
      validationData && tf.tensor2d(validationData.map((d) => d.y));
    ...
    this._log(
      `Start training with ${data.length} datasets (in: ${inputCount} / out: ${outputCount})`
    );
    if (validationData) {
      this._log(
        `validation by ${validationData.length} datasets and minProbability: ${minProbability}`
      );
    }
    ...
    while (!_stop) {
      ...
      const validation = validationData
        ? this.getValidationAccuracy(validationData, minProbability)
        : null;
      const acc = history.val_acc || history.acc;
      const accuracy =
        round(validation?.accuracy || acc[acc.length - 1], 6) -
        (accuracyBaseLine || 0);
      const loss = round(history.loss[history.loss.length - 1], 6);
      const _epochs = counter * epochs;
      const lossIncrease = memHistory
        ? round(loss / memHistory.loss - 1, 6)
        : 0;
      const accuracyIncrease = memHistory
        ? round(accuracy / memHistory.accuracy - 1, 6)
        : 0;
      const ms = new Date() - start;
      const duration = util.humanizeDuration(ms);

      this._log(
        `#${_epochs} ${accuracy.toFixed(6)} / ${loss.toFixed(
          6
        )} after ${duration}`
      );
