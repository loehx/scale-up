...
import { ensure, assert } from "./assertion";
...
export class Log {
  ...
  writeProgress(count, total, debounceMilliseconds = 2000, message) {
    ...
    if (timer) {
      ...
      this.write(
        `${message}${count} of ${total} (${Math.round(
          (count / total) * 100
        )}%) after ${util.humanizeDuration(
          timePassed
        )} (remaining: ${util.humanizeDuration(remaining)})`
      );
    } else {
      this.write(
        `${message}${count} of ${total} (${Math.round((count / total) * 100)}%)`
      );
    }
  }
